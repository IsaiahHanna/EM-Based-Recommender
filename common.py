...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    # Each component has:
    # - d parameters for the mean (mu)
    # - 1 variance parameter (var, scalar for spherical)
    # - 1 mixing proportion (p), but only K-1 are free (they sum to 1)

    num_params = K * d + K + (K - 1)  # d for each mu, 1 for each var, K-1 for mixing weights
    bic_score = log_likelihood - (num_params / 2) * np.log(n)
